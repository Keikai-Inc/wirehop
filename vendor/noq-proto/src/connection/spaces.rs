...
/// Per-path largest-acknowledged packet numbers carried by a sent packet.
///
/// A `TinyVec` rather than an `FxHashMap<PathId, u64>`: the common case is a
/// packet acknowledging just one or two paths, which now needs NO heap
/// allocation. The previous `FxHashMap` allocated a fresh map per *sent* packet
/// and retained it in the in-flight set, multiplied by the number of concurrent
/// multipath paths — a multi-GB leak under load at high path counts. (hop fork.)
#[derive(Debug, Clone, Default)]
pub(super) struct LargestAckedPns(tinyvec::TinyVec<[(PathId, u64); 2]>);

impl LargestAckedPns {
    /// Set the largest acked packet number for `path_id` (replacing any prior).
    pub(super) fn insert(&mut self, path_id: PathId, pn: u64) {
        if let Some(e) = self.0.iter_mut().find(|e| e.0 == path_id) {
            e.1 = pn;
        } else {
            self.0.push((path_id, pn));
        }
    }
    ...
    /// Iterate `(path_id, largest_pn)` — mirrors `FxHashMap::iter`, so call
    /// sites that expect `(&PathId, &u64)` are unchanged.
    pub(super) fn iter(&self) -> impl Iterator<Item = (&PathId, &u64)> {
        self.0.iter().map(|(p, pn)| (p, pn))
    }
}
...
pub(super) struct SentPacket {
    ...
    pub(super) largest_acked: LargestAckedPns,
