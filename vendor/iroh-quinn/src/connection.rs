...
impl Future for ConnectionDriver {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        if conn.error.is_none() {
            // Connection drained without an explicit error being set.
            // This can happen under heavy stream load when a timer-driven
            // transition to Drained state races with the terminate() call.
            // Treat as a local close rather than panicking.
            tracing::warn!("connection drained without error; treating as local close");
