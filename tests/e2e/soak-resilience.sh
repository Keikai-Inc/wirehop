...
for name in "${PERTURBATIONS[@]}"; do
  ...
  read -r p50 p95 mx fails <<EOF
$(python3 - "$RDIR/$name" "$WARM_BUDGET_MS" <<'PY'
import sys
xs=[int(l) for l in open(sys.argv[1]) if l.strip()]
ok=sorted(v for v in xs if v>=0); fails=sum(1 for v in xs if v<0)
def pct(a,p):
    return a[min(len(a)-1, int(round((p/100)*(len(a)-1))))] if a else -1
print(pct(ok,50), pct(ok,95), (max(ok) if ok else -1), fails)
PY
)
EOF
  status="ok"
  [ "$fails" -gt 0 ] && { status="FAIL($fails timeout)"; RC=1; }
  [ "$mx" -gt "$WARM_BUDGET_MS" ] && { status="OVER-BUDGET (max ${mx}ms)"; RC=1; }
  printf "  %-16s p50=%sms p95=%sms max=%sms  %s\n" "$name" "$p50" "$p95" "$mx" "$status"
done
rm -rf "$RDIR"
echo
[ "$RC" = 0 ] && echo "SOAK PASSED (all within ${WARM_BUDGET_MS}ms warm budget)" || echo "SOAK FAILED / over budget — this is the baseline to drive down (see warren-dead-stable.md)"
exit "$RC"
