...
# SLA (the warren dead-stable reliability targets) across the perturbations real users
...
# (≤3s/≤8s) as a later efficiency pass.
...
STAMP=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
{
  ...
  echo "the warren reliability SLA: warm recovery ≤ ${WARM_BUDGET_MS}ms,"
