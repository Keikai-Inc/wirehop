...
#
# Topology:
#   hop-conn-lan   (192.168.77.10)  — plain container on LANNET only (the "LAN
#                                     device"; never runs hop). Not on the warren
#                                     overlay network, so a client can reach it
#                                     ONLY by being routed through the gateway.
#   hop-conn-a     gateway/founder   — on NET (overlay) AND LANNET; advertises
#                                     192.168.77.0/24 (routes.json) + sets up
#                                     ip_forward + nftables masquerade.
#   hop-conn-b     client/member     — on NET only; HOP_ACCEPT_ROUTES=1 installs the
...
cleanup() {
    echo "--- cleanup ---"
    docker rm -f hop-conn-a hop-conn-b hop-conn-lan >/dev/null 2>&1 || true
...
docker run -d --name hop-conn-lan --network "$LANNET" --ip "$LAN_TARGET" "$IMG" sleep infinity >/dev/null
...
docker run -d --name hop-conn-a "${COMMON[@]}" "$IMG" bash -c "
  set -e; mkdir -p /cfg
  echo '192.168.77.10 tablo.test' >> /etc/hosts; hop --config /cfg lan connector tablo.test
...
for i in $(seq 1 30); do
  if docker exec hop-conn-a true 2>/dev/null; then break; fi; sleep 1
done
docker network connect "$LANNET" hop-conn-a

echo "=== waiting for host-a ready (max 120s) ==="
for i in $(seq 1 120); do
  docker exec hop-conn-a test -f /shared/ready-a 2>/dev/null && break; sleep 1
done
docker exec hop-conn-a test -f /shared/ready-a || { echo "FAIL: host-a not ready"; docker exec hop-conn-a cat /cfg/log | tail -30; exit 1; }
echo "--- host-a gateway setup log ---"
docker exec hop-conn-a grep -aE 'vpn gateway|advertis|nftables|ip_forward' /cfg/log | tail -10 || true
docker exec hop-conn-a sh -c 'nft list table inet hop_gw 2>/dev/null' || echo "(WARN: hop_gw nftables table not present)"

echo "=== starting client host-b (HOP_ACCEPT_ROUTES=1) ==="
docker run -d --name hop-conn-b "${COMMON[@]}" -e HOP_ACCEPT_ROUTES=1 "$IMG" bash -c '
  set -e; mkdir -p /cfg
...
for i in $(seq 1 120); do
  docker exec hop-conn-b test -f /shared/ready-b 2>/dev/null && break; sleep 1
done
docker exec hop-conn-b test -f /shared/ready-b || { echo "FAIL: host-b not ready"; docker exec hop-conn-b cat /cfg/log | tail -30; exit 1; }

echo "=== letting netdoc sync + route acceptance + kernel route install settle (35s) ==="
sleep 35

echo "=== diagnostics ==="
echo "--- host-b installed route for $LAN_CIDR? ---"; docker exec hop-conn-b ip route | grep -E '192.168.77|100.64' || true
echo "--- host-b accepted-route / egress debug ---"; docker exec hop-conn-b grep -aE 'vpn route|accepted|egress' /cfg/log | tail -10 || true
echo "--- host-a is on LANNET? ---"; docker exec hop-conn-a ip -o addr show | grep 192.168.77 || true
echo "--- sanity: host-a can reach the LAN device directly ---"
docker exec hop-conn-a ping -c2 -W2 "$LAN_TARGET" >/dev/null 2>&1 && echo "host-a → $LAN_TARGET OK" || echo "WARN: host-a can't reach $LAN_TARGET"

echo "=== member-tunnel sanity: host-b → host-a vIP (isolates tunnel vs routing) ==="
VIP_A=$(docker exec hop-conn-a cat /shared/vip-a 2>/dev/null || true)
echo "host-a vIP: ${VIP_A:-<unknown>}"
if [ -n "$VIP_A" ] && docker exec hop-conn-b ping -c 3 -W 3 "$VIP_A" >/dev/null 2>&1; then
...
for attempt in $(seq 1 6); do
  if docker exec hop-conn-b ping -c 3 -W 3 "$LAN_TARGET"; then
    echo ""; echo "APP-CONNECTOR E2E PASSED: host-b reached a non-hop LAN device via host-a as an app connector (resolved domain → /32)."
    RC=0; break
  fi
...
if [ "$RC" != 0 ]; then
  echo ""; echo "APP-CONNECTOR E2E FAILED: host-b could not reach $LAN_TARGET."
  echo "--- host-a log tail ---"; docker exec hop-conn-a cat /cfg/log | tail -40 || true
  echo "--- host-a nftables ---"; docker exec hop-conn-a nft list ruleset 2>/dev/null | tail -30 || true
  echo "--- host-a ip_forward ---"; docker exec hop-conn-a cat /proc/sys/net/ipv4/ip_forward || true
  echo "--- host-b log tail ---"; docker exec hop-conn-b cat /cfg/log | tail -40 || true
  echo "--- host-b routes ---"; docker exec hop-conn-b ip route || true
  echo "--- host-b egress (debug): does it route the packet to the gateway? ---"
  docker exec hop-conn-b grep -aE 'vpn egress|UNRESOLVED|reach DENIED|192.168.77' /cfg/log | tail -25 || true
  echo "--- host-a ingress (debug): does the packet arrive + get forwarded? ---"
  docker exec hop-conn-a grep -aE 'vpn ingress|DROP|192.168.77|no endpoint→vIP|gateway' /cfg/log | tail -25 || true
  echo "--- host-a conntrack for the target (did a flow form?) ---"
  docker exec hop-conn-a sh -c 'cat /proc/net/nf_conntrack 2>/dev/null | grep 192.168.77 | head' || true
fi
exit "$RC"
