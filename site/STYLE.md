...
7. **Progressive disclosure.** Hero (what it is + the install command) → why →
   the three jobs → deeper (always-on daemon, AI, fleet). Advanced options and
   jargon live later, or behind an "Advanced" disclosure. Don't make the first
   screen carry concepts the reader hasn't earned yet.
